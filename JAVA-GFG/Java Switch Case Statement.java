...
class Solution {
    static double switchCase(int choice, List<Double> arr) {
        switch(choice) {
            case 1:
                double R = arr.get(0);  // Assume arr[0] is radius
                return Math.PI * R * R;
            case 2:
                double L = arr.get(0);  // Assume arr[0] is length
                double B = arr.get(1);  // Assume arr[1] is breadth
                return L * B;
            default:
                return 0.0;  // Handle invalid choice
        }
    }
}
